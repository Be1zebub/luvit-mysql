...
local function istable(var)
	return _istable[type(var)] or false
end
...
function Client:new(conf)
	...
	if conf.logfunc then
		client.logfunc = conf.logfunc else client.logfunc = function() end
	end
	...
	function db:Query(query, args, noquote, debug)
		if args then
			if istable(args) then
				local i = 0
				query = query:gsub("%?", function()
					i = i + 1
					if isstring(args[i]) and noquote ~= true then
		...
		self:query(query, function(err, res, fields)
			if debug then
				p(query)
				p("err", err)
				p("res", res)
			...
			coroutine.resume(running, res, err, fields)
		end)

		return coroutine.yield()
	end

	function db:QueryRow(query, args, row, noquote, debug)
		if row == nil and args then
			row = args
		end
		row = row or 1
		local response = self:Query(query, args, noquote, debug)
		if response then return response[row] end
	end

	function db:QueryValue(query, args, value, noquote, debug)
		local response = self:QueryRow(query, args, nil, noquote, debug)
		if value then
